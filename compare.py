...
**Current dhrystone MIPS** (in commit ${current_hash})**:** ${new_mips}<br/>
**Previous best** (recorded in commit ${best_hash})**:** ${best_mips}, difference ${best_diff}
'''
...
def main(new_file, old_file, current_hash, tolerance, no_update, repo_url):
    ...
    if best_diff < -tolerance:
        ...
        if regressed_hash is None:
            ...
            message = f'⚠ Major regression since commit {f"[{regressed_hash[:8]}](https://github.com/{repo_url}/commit/{regressed_hash})"} ⚠'
        regressed = True
